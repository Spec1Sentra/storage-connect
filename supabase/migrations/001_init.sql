...
-- Views:
--
-- Create public views for exposing limited data
--
...
-- Policies for public.facilities, public.units, public.tags (public read-only)
...
create policy "Public can view images of active items" on public.images for select using (
  exists (select 1 from public.items where items.id = images.item_id and items.status = 'active')
);
create policy "Users can insert images for their own items" on public.images for insert with check (
  auth.uid() = created_by and
  exists (select 1 from public.items where items.id = images.item_id and items.created_by = auth.uid())
);
...
-- Policies for public.item_tags and public.item_embeddings
create policy "Public can view tags/embeddings of active items" on public.item_tags for select using (
  exists (select 1 from public.items where items.id = item_tags.item_id and items.status = 'active')
);
create policy "Users can insert tags for their own items" on public.item_tags for insert with check (
  exists (select 1 from public.items where items.id = item_tags.item_id and items.created_by = auth.uid())
);
create policy "Public can view tags/embeddings of active items" on public.item_embeddings for select using (
  exists (select 1 from public.items where items.id = item_embeddings.item_id and items.status = 'active')
);
create policy "Users can insert embeddings for their own items" on public.item_embeddings for insert with check (
  exists (select 1 from public.items where items.id = item_embeddings.item_id and items.created_by = auth.uid())
);

-- Policies for public.claims
create policy "Participants can view claims" on public.claims for select using (
  claimant_id = auth.uid() or
  exists (select 1 from public.items where items.id = claims.item_id and items.created_by = auth.uid())
);
...
create policy "Participants can view conversations" on public.conversations for select using (
  claimant_id = auth.uid() or item_owner_id = auth.uid()
);
create policy "Claimant can create conversation" on public.conversations for insert with check (
  claimant_id = auth.uid()
);

-- Policies for public.messages
create policy "Participants can view messages" on public.messages for select using (
  exists (select 1 from public.conversations where conversations.id = messages.conversation_id and (conversations.claimant_id = auth.uid() or conversations.item_owner_id = auth.uid()))
);
...
-- Policies for public.saved_searches
...
-- Policies for public.push_tokens
